		...
		public void onBackStackChanged() {
			if (getChildFragmentManager().getBackStackEntryCount() == 0) {
				...
				for (Fragment childFragment : getChildFragmentManager()
						.getFragments()) {
					if (childFragment instanceof me.kuehle.carreport.gui.DataListListener) {
						((me.kuehle.carreport.gui.DataListListener) childFragment).unselectItem();
					}
				}
			} else {
		...
		public boolean onNavigationItemSelected(int itemPosition, long itemId) {
			...
			for (Fragment fragment : getChildFragmentManager().getFragments()) {
				if (fragment instanceof me.kuehle.carreport.gui.DataListListener) {
					((me.kuehle.carreport.gui.DataListListener) fragment).onCarChanged(mCurrentCar);
				}
			}

...
public class TireListViewModel extends AndroidViewModel {
    private final AutuManduDatabase db;
    private final MutableLiveData<Long> carId = new MutableLiveData<>();

    public TireListViewModel(@NonNull Application application) {
        super(application);
    ...
    public LiveData<List<TireWithDetails>> getTires() {
        return Transformations.switchMap(carId, id -> {
            if (id == null || id == -1) {
                return new MutableLiveData<>(null);
            }
            return db.getTireDao().getAllForCarWithDetailsLiveData(id);
        });
    }

    public void delete(long id) {
        AutuManduDatabase.DB_EXECUTOR.execute(() -> db.getTireDao().deleteTireListById(id));
    }
}

            ...
            isGranted -> {
                if (isGranted) {
                    if (mPendingPermissionRequestCode == REQUEST_AUTO_BACKUP_PERMISSIONS) {
                        SwitchPreferenceCompat autoBackup = findPreference("behavior_auto_backup");
                        if (autoBackup != null) {
    ...
    private final ActivityResultLauncher<Intent> mSetupSyncLauncher = registerForActivityResult(
            ...
            result -> {
                if (result.getResultCode() == android.app.Activity.RESULT_OK && result.getData() != null) {
                    ...
                    java.security.cert.X509Certificate cert = (java.security.cert.X509Certificate) data.getSerializableExtra("webdav_certificate");
    ...
    private void performActionAfterPermissionGranted(int requestCode) {
        ...
        switch (requestCode) {
            case REQUEST_BACKUP_PERMISSIONS -> {
                if (new Backup(context).backup(false)) {
                    Toast.makeText(context, R.string.toast_backup_succeeded, Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(context, R.string.alert_backup_failed, Toast.LENGTH_SHORT).show();
                }
            }
            case REQUEST_RESTORE_PERMISSIONS -> {
                ...
                if (backup.backupFileExists()) {
                    ...
                    Toast.makeText(context, R.string.pref_summary_restore_file_seems_wrong, Toast.LENGTH_SHORT).show();
                }
            }
            case REQUEST_EXPORT_PERMISSIONS -> {
                try {
                    new CSVExportImport(context).export();
                    Toast.makeText(context, R.string.toast_export_csv_succeeded, Toast.LENGTH_SHORT).show();
                } catch (Exception e) {
                    Toast.makeText(context, R.string.alert_export_csv_failed, Toast.LENGTH_SHORT).show();
                }
            }
            case REQUEST_IMPORT_PERMISSIONS -> {
                CSVExportImport csvExportImport = new CSVExportImport(context);

CREATE TABLE othercosts2 ( 
	...
	tachometer INTEGER NOT NULL DEFAULT -1,
	price REAL NOT NULL,
	repeat_interval INTEGER NOT NULL DEFAULT 0,
	repeat_multiplier INTEGER NOT NULL DEFAULT 1,
	note TEXT NOT NULL,
	cars_id INTEGER NOT NULL,
	FOREIGN KEY(cars_id) REFERENCES cars(_id) ON UPDATE CASCADE ON DELETE CASCADE);
INSERT INTO othercosts2 (_id, title, `date`, tachometer, price, repeat_interval, repeat_multiplier, note, cars_id)
	SELECT othercosts._id, title, `date`, tachometer, price, repeat_interval, repeat_multiplier, note, cars_id
	FROM othercosts
	JOIN cars ON cars._id = othercosts.cars_id;

...
public class DataDetailCarFragment extends AbstractDataDetailFragment implements
	...
	private View addFuelTypeView(FuelType fuelType) {
		...
		ArrayAdapter<String> nameAdapter = new ArrayAdapter<String>(
				...
				fuelTypeNames);
		edtName.setAdapter(nameAdapter);
		...
		for (int i = 1; i <= 2; i++) {
			tankAdapter.add(getString(R.string.label_tank, i));
		}
		spnTank.setAdapter(tankAdapter);
	...
	protected void initFields(View v) {
		...
		fuelTypeNames = FuelType.getAllNames();
